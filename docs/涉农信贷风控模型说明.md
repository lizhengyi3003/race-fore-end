...
> 版本：2026-08 · 对应 `back-end/app/ml/` 与 `back-end/app/services/risk_service.py`
...
- **多元统计评分卡**（`scorecard.py` + `predictor.py`）：经典 Logistic 评分卡（IV 筛选 → WOE 编码 → VIF 诊断 → 回归），作为参考模型与演示基线。
...
指标依据《农业及相关产业统计分类（2020）》与东北振兴区域涉农信贷场景编制（见 `docs/农业及相关产业动态指标搜集体系.md`，共 700+ 条），按**树形分层**组织：

| 层级 | 说明 | 示例 |
| --- | --- | --- |
| 基本项 | 所有主体必填的通用信息（约 30 项） | 实际经营年限、年营业收入、资产负债率、银行信贷履约记录 |
| 大类（2 位码） | 01~10 十大农业及相关产业 | 01 农林牧渔业、02 农产品加工、05 农产品流通… |
| 中类（3 位码） | 大类下的细分 | 011 农业生产、012 林业生产… |
| 小类（4 位码） | 中类下的细分 | 0111 谷物种植、0112 豆类油料… |
| 具体营业类型（叶子，`小类码_行业码`） | 最小可勾选经营单元 | 0111_0111 稻谷种植、0111_0113 玉米种植… |
...
| 多元统计评分卡 | `back-end/app/ml/scorecard.py`、`predictor.py` |
